...
Now that the code is uploaded we need to go to `File` -> `Save` to save the function. Now we can deploy it. Click on the **Deploy** button and wait for it to finish. We can move onto creating the API Gateway.
