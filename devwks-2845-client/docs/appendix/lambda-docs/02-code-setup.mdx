...
Select the **Environment variables** tab and then **Edit** it. Click on **Add environment variable**.

Name the `key` **`XCiscoMerakiAPIKey`**.

Then enter your Meraki API key as the `Value`.

Then **Save** the configuration on the bottom right. It will look similar to this image except your `Value` will have something there:

![Environment variable](../../../static/img/edit-environ.png)

## Change the Function Timeout

While in the **Configuration** section, we need to update the **General configuration** tab. Click on the **General configuration** tab and then click on **Edit** on the right side. Scroll down to the **Timeout** field and change it to `60` seconds. It will look like the below image:
![timeout](../../../static/img/timeout.png)
...
The `Zip` option requires that we create a local folder for our code.

On your computer please create a new folder named **shutdownPort** using your preferred method (CLI or GUI).
...
In the local code editor, create a new file called **`lambda_function.py`** and paste the code from the Lambda function into it.
