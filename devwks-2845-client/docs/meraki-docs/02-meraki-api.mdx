...
We are using the below code to create our Custom Webhook Template. Copy the code below using the copy button on the top right of the code block. Create a new file in the Code Editor called `UploadCustomWebhookTemplate.py` and paste the code you copied into it. Then save the file using `CTRL+S` or by going to `File -> Save` on the top left of the code editor window.
...
Open the **Terminal** application, if it is not already open in the code editor, and change directory to where you saved **UploadCustomWebhookTemplate.py** as above
...
Give the webhook a meaningful name. A suggestion is to use **_TraineeXX_** where **_XX_** is your trainee number.

We need to type in the following to run the code:

`python3 UploadCustomWebhookTemplate.py -k ***YOURAPIKEY*** -n ***YOURNETWORKID*** -i "TraineeXX"`
...
Under **_Payload Template_** click the drop down box, and you should see something similar to what is in the image below:
