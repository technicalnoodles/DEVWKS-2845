...
Open the **Terminal** application, if it is not already open in the code editor, and change the directory to where you saved **UploadCustomWebhookTemplate.py** if the terminal is not already there.
...
The below is how to run the command and you will need to replace each stared word (and the stars) with the subsequent data you got earlier:
