...
Looking through the above JSON, we know we need the serial number, port number, alert type, and the status of the port.
...
We will add the greeen highlighted lines to the `lambda_handler` and remove the red ones.
